...
def test_vip_1(page, keyword, case_id, request):
    request.node._testrail_case_id = case_id
    etc = Etc(page)
    vip_page = Vip(page)
    logger = TimeLogger("test_vip.json")
    # vip 로 이동
    etc.goto_vip()
...
@pytest.mark.flaky(reruns=2, reruns_delay=1)
@pytest.mark.parametrize("keyword, case_id", vip_testcases2, ids=[c for _, c in vip_testcases2])
def test_vip_2(page, keyword, case_id, request):
    request.node._testrail_case_id = case_id
    logger = TimeLogger("test_vip.json")
    etc = Etc(page)
    vip_page = Vip(page)
